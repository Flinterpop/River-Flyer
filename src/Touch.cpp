...
// JAM: alone, it sits in the bottom row left of CHAFF, because stacked above
// FIRE it reaches 5.1 u up and a tall phone's bar under the canvas is only
// about 3.6 u, so it would land on the river. With two players each half is
// only 6 u wide, too narrow for a row of three, so it goes back above FIRE.
Circle JamBtn(int pilot)
{
    const float u = Unit();
    const float h = static_cast<float>(GetScreenHeight());
    if (players < 2) { return Circle {{SideX(pilot, 6.3f * u), h - 1.5f * u}, kSmallR * u}; }
    return Circle {{SideX(pilot, 1.7f * u), h - 4.3f * u}, kSmallR * u};
}
